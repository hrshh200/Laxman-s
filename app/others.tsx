...
export default function Paan() {
    ...
    const [paanCategories, setPaanCategories] = useState<OtherItem[]>([]);
    const { user, logout } = useAuth();

    const isOthersAvailableNow = () => {
        const now = new Date();
    ...
    const [isAvailableNow, setIsAvailableNow] = useState(isOthersAvailableNow());

    const handleAddToCart = (item: OtherItem) => {
        router.push({
    ...
    useEffect(() => {
        const fetchOthersData = async () => {
            try {
                ...
                setPaanCategories(data);
            } catch (error) {
                console.error('Error fetching paan data:', error);
            }
        };
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setIsAvailableNow(isOthersAvailableNow());
        ...
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.header}>
                ...
                <Text style={styles.headerTitle}>
                    <Text style={styles.greenPaan}>Other items</Text>
                </Text>
            ...
            <ScrollView showsVerticalScrollIndicator={false} style={styles.scrollView}>
                ...
                <View style={styles.section}>
                    {paanCategories.length === 0 ? (
                        <View style={styles.emptyContainer}>
                            <Text style={styles.emptyText}>No items present</Text>
                        </View>
                    ) : (
                        paanCategories.map((item) => (
